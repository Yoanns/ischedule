...
if ($_POST['PhotoAvant'] != '' && @$_POST['PHOTOdelete'] == 'ON')
{
	...
	@unlink($DossierNewsPhoto.$_POST['PhotoAvant']);
	// Suppression dans la base de donnees par UPDATE
	mysql_query("UPDATE notes SET PhotoNt='' WHERE IdNt= ".$notesID);
}
// ----------------------------------
...
if(isset($_FILES['PHOTO']))
{
	...
	if ($traiter_photo_OK == 'OK')
	{
		if($_FILES['PHOTO']['size']>0)
		{
			...
			move_uploaded_file($temp, $DossierNewsPhoto.$PHOTOupload);
			// --------------------
			...
			$redimPHOTOOK = fctredimimage($photolargeur,0,'','',$DossierNewsPhoto,$PHOTOupload);
			// --------------------
			...
			mysql_query("UPDATE notes SET PhotoNt='".$PHOTOupload."' WHERE IdNt= '".$notesID."'");
		} 
	}
