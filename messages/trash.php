	...
	if (isset($_POST["btDEL"]))
{
	...
	$query_delete = 	"DELETE FROM messages WHERE IdMsg='".$IdMsg."';";
...
$targetpage = $_SERVER["PHP_SELF"]; 	
	...
	$query = "SELECT COUNT(*) AS num 
				FROM messages 
				WHERE `ToEmp` = '$ToEmp' AND `Deleted` = '1' ";
   ...
   // Get page data
   $msg_query = "SELECT * FROM messages WHERE `ToEmp` = '$ToEmp' AND `Deleted` = '1' ORDER BY `DateSent` DESC LIMIT $start, $limit";
...
<head>
	...
	<title>Shift-Scheduler.com | Trash</title>
				...
				while($msg_row = mysql_fetch_array($msg_result)) 
					{
        				...
        				<tr>
							<td><form method="post" name="formsupprimer" action="trash.php" onClick="return confirm('Do you really want to delete this message definitely?')">
		<input type="hidden" name="IdMsg" value="<?php echo $IdMsg; ?>" />		
		<button name="btDEL" type="submit" title="Delete message" class="button"><img src="../Admin/icones/bin_empty.png" width="24" alt="Delete message" /></button>
	</form></td>
...
<a href="../empl_site.php?page=1#simpleContained3" title="Go back"><div class="large button red"><img src="../Admin/icones/arrow_back.png" alt="Go back" />Go back</div></a>	
 </div>
 </div>
